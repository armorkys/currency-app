...
class MainControllerTest {
	...
	@Autowired
	public MainControllerTest(RestTemplateBuilder restTemplateBuilder) {
	...
	void getCurrencyHistory() throws DatatypeConfigurationException, JsonProcessingException {
		...
		server.expect(ExpectedCount.once(),
				requestTo(FxClient.URL_MAIN + "getFxRatesForCurrency?tp=EU&ccy=" + CcyISO4217.USD + "&dtFrom="
						+ LocalDate.now().minusYears(1) + "&dtTo=" + LocalDate.now()))
				.andExpect(method(HttpMethod.GET))
				.andRespond(withSuccess(expectedReturnBody, MediaType.APPLICATION_XML));
		...
		assertThat(actualCurrencyRatesList.get(0).getTp()).isEqualTo(FxRateTypeHandling.EU);
		assertThat(actualCurrencyRatesList.get(0).getCcyAmt().get(1).getCcy()).isEqualTo(CcyISO4217.USD);
	}
	...
	void getCurrencyHistoryCustom() throws DatatypeConfigurationException, JsonProcessingException {
		...
		String expectedReturnBody = xmlMapper.writeValueAsString(fxRatesExpected);
		DateHistoryTemplate urlInput = new DateHistoryTemplate(CcyISO4217.USD, LocalDate.parse("2020-01-01"),
				LocalDate.parse("2020-02-10"));

		server.expect(ExpectedCount.once(),
				requestTo(FxClient.URL_MAIN + "getFxRatesForCurrency?tp=EU&ccy=" + urlInput.getCcy() + "&dtFrom="
						+ urlInput.getStartDate() + "&dtTo=" + urlInput.getEndDate()))
				.andExpect(method(HttpMethod.GET))
				.andRespond(withSuccess(expectedReturnBody, MediaType.APPLICATION_XML));
		DateHistoryTemplate dateTemplate = new DateHistoryTemplate(CcyISO4217.USD, urlInput.getStartDate(),
				urlInput.getEndDate());
	...
	void getCurrencyHistoryCustomWithErrors() throws DatatypeConfigurationException, JsonProcessingException {
		...
		bindingResult.addError(new FieldError("dateInputTemplate", "endDate", "Bad end date"));
		bindingResult.addError(new FieldError("dateInputTemplate", "startDate", "Bad start date"));
	...
	void convertCurrencyPost() throws DatatypeConfigurationException, JsonProcessingException {
		CcyComparator expectedComparator = createCcyComparator2(BigDecimal.ONE, BigDecimal.valueOf(1.2),
				BigDecimal.TEN);
